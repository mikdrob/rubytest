...
class TestCourseMethods < Minitest::Test
  ...
  def test_attr_reader
    assert_equal(@course_name, @course.name, 'Check Course name correctness.')
    assert_equal(@today, @course.start_date,
                 'Check Course start_date correctness.')
  end
...
class TestCourseEnroll < Minitest::Test
  ...
  def test_enrol_fail
    @course_past.enrol(@student1)
    err_string = $stderr.string.gsub(/[\r\n]/, '')
    # rubocop: disable Layout/LineLength
    assert_equal("Can not enrol #{@student1_name}. #{@course_past_name} start is in past.",
                 err_string, 'Check enrol exception handler correctness.')
    # rubocop: enable Layout/LineLength
    assert_equal(false, @course_past.enrolled?(@student1),
                 'Check enrolled? method correctness.')
  end

  def test_enrol_success
    @course_future.enrol(@student1)
    assert_equal(true, @course_future.enrolled?(@student1),
                 'Check enrolled? method correctness.')
  end

  def test_enrolled
    @course_future.enrol(@student1)
    @course_future.enrol(@student2)
    assert_equal([@student1, @student2], @course_future.enrolled_students,
                 'Check enrolled_students method correctness.')
  end
end
